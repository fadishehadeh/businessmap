...
const ServiceTile = ({ service, isActive, onClick }: ServiceTileProps) => {
  return (
    ...
    >
      ...
      <div className="absolute inset-0 flex flex-col items-start justify-end pb-8 p-6 text-left">
        ...
        <p className="text-white text-xl max-w-[80%] opacity-90 transition-opacity duration-500 group-hover:opacity-0">
          {service.description}
        </p>
      </div>
      ...
      {/* Hover State Content */}
      <div className={cn(
        "absolute inset-0 flex flex-col items-start justify-end pb-8 p-6",
        "bg-black/40 backdrop-blur-sm",
        "opacity-0 scale-95 group-hover:opacity-100 group-hover:scale-100",
        "transition-all duration-500 ease-in-out"
      )}>
        <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20 shadow-xl w-[80%] text-left">
          {/* Description */}
          ...
          <div className="flex justify-start gap-4 mb-4">
            {Object.entries(service.stats).slice(0, 1).map(([key, value], idx) => (
              <div key={idx} className="text-left">
                <p className="text-white/70 text-xl capitalize">
